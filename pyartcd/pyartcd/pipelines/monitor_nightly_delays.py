...
class MonitorNightlyDelaysPipeline:
    ...
    def __init__(
        ...
        self.release_artists_channel = release_artists_channel or f"#art-release-{self._ocp_version}"

        # Redis keys for tracking alert state
        self._redis_key_6h = f"nightly_delay_alert_6h:{self.group}"
        self._redis_key_12h = f"nightly_delay_alert_12h:{self.group}"
    ...
    async def _alert_12h(
        ...
        message = (
            ...
            f"Created: {nightly_time.strftime('%Y-%m-%d %H:%M:%S UTC')} "
            f"(*{hours_since_last:.1f} hours ago*)\n\n"
            f"TRT team please investigate immediately."
        )
...
@click.option(
    '--trt-channel',
    default='#forum-trt',
    help='Slack channel to ping TRT team (default: #forum-trt)',
)
@pass_runtime
@click_coroutine
